#include <assert.h>
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <openssl/ssl.h>
#include <openssl/err.h>

#include "api.h"
#include "ui.h"
#include "util.h"

struct client_state {
  struct api_state api;
  int eof;
  struct ui_state ui;
  SSL_CTX *ssl_ctx;
  SSL *ssl;
};


int create_user(const char *msg){

  char buf[strlen(msg)+1];
  strncpy(buf,msg,sizeof(buf)-1);
  buf[sizeof(buf)-1] = '\0';

  char *saveptr = NULL;
  strtok_r(buf, " \t", &saveptr);
  char *username = strtok_r(NULL, " \t", &saveptr);

  if(!username){
    fprintf(stderr, "error: missing username\n");
    return 0;
  }

  char fileName[strlen(username) + 32];
  snprintf(fileName, sizeof(fileName), "clientkeys/%s-key.pem",username);

  if(access(fileName,F_OK) == 0){
    fprintf(stderr, "error: username already exists\n");
    return 0;
  }

  char cmd[strlen(username) + 32];
  snprintf(cmd, sizeof(cmd), "./ttp.sh create-user %s", username);
  printf("%s", cmd);
  system(cmd);
  
  return 1;
}


/**
 * @brief Connects to @hostname on port @port and returns the
 *        connection fd. Fails with -1.
 */
static int client_connect(struct client_state *state,
  const char *hostname, uint16_t port) {
  int fd;
  struct sockaddr_in addr;

  assert(state);
  assert(hostname);

  /* look up hostname */
  if (lookup_host_ipv4(hostname, &addr.sin_addr) != 0) return -1;
  addr.sin_family = AF_INET;
  addr.sin_port = htons(port);

  /* create TCP socket */
  fd = socket(AF_INET, SOCK_STREAM, 0);
  if (fd < 0) {
    perror("error: cannot allocate server socket");
    return -1;
  }

  /* connect to server */
  if (connect(fd, (struct sockaddr *) &addr, sizeof(addr)) != 0) {
    perror("error: cannot connect to server");
    close(fd);
    return -1;
  }

  return fd;
}

static int client_process_command(struct client_state *state) {

  assert(state);

  char buf[API_MAX_CLIENT_MSG_LEN+1];

  ssize_t n = read(STDIN_FILENO, buf, sizeof(buf)-1);

  if(n < 0 ){
    perror("error: reading from stdin");
  }
  
  buf[n] = '\0';
  buf[strcspn(buf, "\n")] = '\0';  // Remove newline character

  if(n > API_MAX_CLIENT_MSG_LEN){
    printf("error: maximum message limit exceeded");
    return 0;
  }
  
  if (n <= 0 || strcmp(buf, "/exit") == 0) {
      printf("Exiting Program...\n");
      state->eof = 1;  // Ctrl+D pressed
  }
  else if (strncmp(buf, "/login ", 7) == 0){
    if (api_send(&state->api, buf) < 0) {
        perror("error: failed to send register command\n");
        return -1;
    }
  }
  else if(strncmp(buf, "/register ",10) == 0){

    if(!create_user(buf)){
      perror("error: user already exists");
      return -1;
    }

    if (api_send(&state->api, buf) < 0) {
        perror("error: failed to send register command\n");
        return -1;
    }
    
  }
  else if(strcmp(buf, "/users") == 0){
    if (api_send(&state->api, buf) < 0) {
        perror("error: failed to send register command\n");
        return -1;
    }

  }
  else if(strncmp(buf, "/", 1) == 0){
    printf("error: command not recognized\n");
  }
  else if(strncmp(buf, "@", 1) == 0){
    printf("error: private messaging not avaliable\n");

  }
  else{

    if(api_send(&state->api, buf) < 0){
      perror("error: failed to send message\n");
      return -1;
    }

  }
      
  return 0;
}

/**
 * @brief         Handles a message coming from server (i.e, worker)
 * @param state   Initialized client state
 * @param msg     Message to handle
 */
static int execute_request(
  struct client_state *state,
  const struct api_msg *msg) {

  printf( "%s\n", msg->data);

  return 0;
}

/**
 * @brief         Reads an incoming request from the server and handles it.
 * @param state   Initialized client state
 */
static int handle_server_request(struct client_state *state) {
  struct api_msg msg;
  int r, success = 1;

  assert(state);

  /* wait for incoming request, set eof if there are no more requests */
  r = api_recv(&state->api, &msg);
  if (r < 0) return -1;
  if (r == 0) {
    state->eof = 1;
    return 0;
  }

  /* execute request */
  if (execute_request(state, &msg) != 0) {
    success = 0;
  }

  /* clean up state associated with the message */
  api_recv_free(&msg);

  return success ? 0 : -1;
}

/**
 * @brief register for multiple IO event, process one
 *        and return. Returns 0 if the event was processed
 *        successfully and -1 otherwise.
 *
 */
static int handle_incoming(struct client_state *state) {
  int fdmax, r;
  fd_set readfds;

  assert(state);



  /* list file descriptors to wait for */
  FD_ZERO(&readfds);
  FD_SET(STDIN_FILENO, &readfds);
  FD_SET(state->api.fd, &readfds);
  fdmax = state->api.fd;

  /* wait for at least one to become ready */
  r = select(fdmax+1, &readfds, NULL, NULL, NULL);
  if (r < 0) {
    if (errno == EINTR) return 0;
    perror("error: select failed");
    return -1;
  }

  /* handle ready file descriptors */
  if (FD_ISSET(STDIN_FILENO, &readfds)) {
    return client_process_command(state);
  }

  if (state->api.use_tls && SSL_pending(state->api.ssl) > 0) {
    return handle_server_request(state);
  }

  
  return 0;
}

static int client_state_init(struct client_state *state) {
  /* clear state, invalidate file descriptors */
  memset(state, 0, sizeof(*state));

  /* initialize UI */
  ui_state_init(&state->ui);

 

  return 0;
}

static void client_state_free(struct client_state *state) {


  /* cleanup API state */
  api_state_free(&state->api);

  /* cleanup UI state */
  ui_state_free(&state->ui);
}

static void usage(void) {
  printf("usage:\n");
  printf("  client host port\n");
  exit(1);
}

int main(int argc, char **argv) {
  int fd;
  uint16_t port;
  struct client_state state;

  /* check arguments */
  if (argc != 3) usage();
  if (parse_port(argv[2], &port) != 0) usage();

  /* preparations */
  client_state_init(&state);

  /* connect to server */
  fd = client_connect(&state, argv[1], port);
  if (fd < 0) return 1;

  /* initialize API */
  api_state_init(&state.api, fd);

  setvbuf(stdout, NULL, _IONBF, 0);



  SSL_library_init();
  SSL_load_error_strings();
  OpenSSL_add_all_algorithms();

  SSL_CTX *ssl_ctx = SSL_CTX_new(TLS_client_method());
  if (!ssl_ctx){
    fprintf(stderr, "TLS error\n");
    return 1;
  }

  SSL_CTX_set_verify(ssl_ctx, SSL_VERIFY_PEER, NULL);

  if (!SSL_CTX_load_verify_locations(ssl_ctx, "clientkeys/ca-cert.pem", NULL)) {
    fprintf(stderr, "TLS error: unable to load certification\n");
    return 1;
  
  }
  SSL *ssl = SSL_new(ssl_ctx);
  SSL_set_fd(ssl,fd);

  if (SSL_connect(ssl) <= 0){
    fprintf(stderr, "TLS handshake failed\n");
    ERR_print_errors_fp(stderr);
    return 1;
  }

  printf("TLS handshake succesful\n");

  state.api.ssl =ssl;
  state.api.use_tls = 1;
  
  /* client things */
  while (!state.eof && handle_incoming(&state) == 0);

  /* clean up */
  client_state_free(&state);
  close(fd);

  return 0;
}

