# Secure Programming Test Suite Information

This file contains information about the Secure Programming automated
test script suite. We suggest you read it prior to testing your homework
...
2. A test will check the stderr of all launched programs upon the completion
   ...
   input to /dev/null if you're calling openssl routines from your makefile.
...
All predefined messages are explained below.

"registration succeeded" should be shown standalone on a newline if the 
 user successfully registers with a @username and @password.

"error: user erik already exists" should be written standalone on a newline 
 if we cannot register user @erik as he was already registered.

"authentication succeeded" should be written standalone on a newline if the 
 user successfully logins with a @username and @password.

"error: invalid credentials" should be written standalone on a newline if the 
 user cannot login due to invalid credentials.

"error: unknown command /command" should be written standalone on a newline 
if the user types in a @command not included in the assignment sheet.

"error: invalid command format" should be written standalone on a newline 
if the user types in an existing command without adhering to the syntax 
specified in the assignment sheet.

"error: user not found" should be written standalone on a newline 
if we try to send a private message to an unregistered user.

"error: command not currently available" should be written standalone 
on a newline if user types in a command that shouldn't be available while
the user is logged in/out.

## Notes

1. all the success/error messages should be written to stdout.
2. you can output any other additional messages in your app provided
   ...
   accordingly.
3. we will also manually verify your homework so do not try to game
   the script.
