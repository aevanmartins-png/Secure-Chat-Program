...
CLIENT_COUNT_MAX = 3
CLIENT_DIRS = [ "clientkeys", "ttpkeys" ]
CLIENT_FILES = [ "client", "ttp.sh" ]
...
class CommandResult:
  ...
  def report(self, success):
    ...
    sys.stdout.write(" " + ("success" if success else "failed") + "\n")
    self.is_done = True
...
def run_make(dir, target):
  ...
  sys.stdout.write(f"running make {target}...")
  ...
  try:
    ...
    sys.stderr.write(f"error: failed to start make: {e.strerror}\n")
    sys.exit(2)

  if result.returncode != 0:
    cmdresult.report(False)
    if result.returncode < 0:
      sys.stderr.write(f"error: make failed with signal {-result.returncode}\n")
    else:
      sys.stderr.write(f"error: make failed with exit code {result.returncode}\n")
...
def callback_check_pubmsg_recv(pm):
  # executed with one client
  pm.sendinput(1, f"/login erik {PASSWORD1}\n")
  pm.sendinput(2, f"/register user2 {PASSWORD2}\n")
...
def callback_check_privmsg_send(pm):
  ...
  pm.sendinput(3, f"/register sam {PASSWORD3}\n")
  pm.sendinput(1, f"/login erik {PASSWORD1}\n")
...
def callback_check_privmsg_retr(pm):
  ...
  # executed with three clients
  pm.sendinput(1, f"/login erik {PASSWORD1}\n")
  pm.sendinput(2, f"/login user2 {PASSWORD2}\n")
  pm.sendinput(3, f"/login sam {PASSWORD3}\n")
  pm.waitall()
  pm.matchoutput(1, [REGEXP_TIMESTAMP+" erik: @user2 this is privmsg1"], True)
...
def callback_check_users_two(pm):
  pm.sendinput(1, f"/login erik {PASSWORD1}\n")
  pm.sendinput(2, f"/register manuel {PASSWORD3}\n")
  pm.sendinput(1, "/users\n")
  pm.waitall()
...
def callback_check_users_three(pm):
  ...
  pm.sendinput(1, "/users\n")
  pm.waitall()
  pm.matchoutput_noorder(1, ["erik", "user2", "sam"], True)
...
def interact(sourcedir, tmpdir, clientcount, callback, msg):
  ...
  testindex += 1
  tmpdirtest = os.path.join(tmpdir, f"test{testindex}")

  # create all processes
  pm = procman.ProcessManager(sourcedir, tmpdir, tmpdirtest, get_tcpport(), clientcount, cmdresult)
  try:
...
def check_build(dir):
  ...
  if not os.path.isfile(serverpath):
    cmdresult.report(False)
    die(f"file {serverpath} does not exist")

  clientpath = os.path.join(dir, "client")
  if not os.path.isfile(clientpath):
    cmdresult.report(False)
    die(f"file {clientpath} does not exist")
...
def check_clean(dir):
  ...
  for name in os.listdir(dir):
    if name in ["server", "client", "chat.db"] or name.endswith(".o"):
      cmdresult.report(False)
      sys.stderr.write(f"warning: file {os.path.join(dir, name)} remains after clean\n")
      warnings = True
...
if not os.path.isdir(sourcedir): die(f"directory {sourcedir} does not exist")

makefile = os.path.join(sourcedir, "Makefile")
if not os.path.isfile(makefile): die(f"file {makefile} does not exist")
...
for i in range(CLIENT_COUNT_MAX):
  ...
  for dirname in CLIENT_DIRS:
    shutil.copytree(dirname, os.path.join(clientdir, dirname))

  for filename in CLIENT_FILES:
    shutil.copy2(filename, clientdir)
...
interact(sourcedir, tmpdir, 3, callback_check_privmsg_send, "testing simple private message send...")
...
interact(sourcedir, tmpdir, 1, gen_callback_check_invalid_command("/loginer", ["erik", PASSWORD1]), "testing unknown command /loginer...")
