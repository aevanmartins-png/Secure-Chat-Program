...
static int server_state_init(struct server_state *state) {
  ...
  //Load OpenSSL objects
  state->server_privatekey = read_private_key("serverkeys/server-key.pem");
  if(!state->server_privatekey) return -1;

  state->server_cert = read_certificate("serverkeys/server-cert.pem");
  if(!state->server_cert) return -1;

  state->ca_cert = read_certificate("serverkeys/ca-cert.pem");
...
static void server_state_free(struct server_state *state) {
  ...
  if(state->server_privatekey){
    EVP_PKEY_free(state->server_privatekey);
  }
  if(state->server_cert){
    X509_free(state->server_cert);
  }
  if(state->ca_cert){
    X509_free(state->ca_cert);
  }
...
static EVP_PKEY *read_private_key(const char *path){
  ...
  FILE *f = fopen(path,"r");
  if(!f){
    perror("error: Couldnt open private key file");
    return NULL;
  }
  ...
  if(!pkey){
    perror("error: Couldn't read server private key");
    return NULL;
  }
...
static X509 *read_certificate(const char *path){
  ...
  FILE *f = fopen(path, "r");
  if(!f){
    perror("error: Couldnt open certificate file");
    return NULL;
  }
  ...
  if(!cert){
    perror("error: Couldnt read certificate");
    return NULL;
  }
...
EVP_PKEY *get_pubkey_from_cert(X509 *cert){
  ...
  if(!cert){
    perror("error: Couldnt get certificate");
    return NULL;
  }
  ...
  if(!pkey){
    perror("error: Couldn't retrieve public key");
  }
